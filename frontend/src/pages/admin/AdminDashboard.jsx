...
export default function AdminDashboard() {
  ...
  const handleAddStation = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        showToast("success", "Station Added", `Station ${formData.stationId} created successfully.`);
        ...
        showToast("error", "Failed to Add", error.message || "Something went wrong.");
      }
    } catch (err) { showToast("error", "Error", "Failed to add station."); }
  };
  ...
  const handleEditStation = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        showToast("success", "Station Updated", `Station ${editingStation.station_id} updated successfully.`);
        setShowEditModal(false);
        setEditingStation(null);
        fetchStations();
      }
    } catch (err) { showToast("error", "Error", "Failed to update station."); }
  };
  ...
  const handleApprove = async (notification) => {
    ...
    try {
      ...
      const data = await response.json();
      if (response.ok) {
        showToast("success", "Password Reset Successful", `Generated password is ${data.newPassword}. Sent to ${notification.email}.`);
        setShowNotifications(false);
        fetchNotifications();
      } else { showToast("error", "Error", data.message); }
    } catch (err) { showToast("error", "Network Error", "Failed to communicate with the server."); }
  ...
  const updateQuota = async (rule) => {
    try {
      ...
      if (response.ok) {
        showToast("success", "Updated", "Quota rule updated successfully.");
        fetchQuotaRules();
      }
    } catch (err) { showToast("error", "Error", "Failed to update quota."); }
  };
  ...
  const toggleStationStatus = async (stationId, currentStatus) => {
    ...
    try {
      ...
      if (response.ok) {
        showToast("success", "Updated", `Station ${stationId} is now ${newStatus}.`);
        fetchStations();
        fetchStats();
      }
    } catch (err) { showToast("error", "Error", "Failed to update station status."); }
  };
  ...
  return (
    <div className="min-h-screen bg-[#0B1220] text-white flex">
      {/* Sidebar */}
      <div className="w-64 border-r border-white/10 flex flex-col p-6 gap-2 overflow-hidden">
        ...
        {[
          ...
          { id: "quota", label: "Quota Rules", icon: Settings },
          { id: "stations", label: "Stations", icon: MapPin },
          { id: "supply", label: "Station Supply", icon: Fuel },
          ...
          <button
            ...
            className={`flex items-center gap-3 px-4 py-3 rounded-xl transition cursor-pointer ${
              activeTab === tab.id ? "bg-fuchsia-500/10 text-fuchsia-400 border border-fuchsia-500/20" : "text-gray-400 hover:bg-white/5"
            }`}
          >
            <tab.icon size={20} />
        ...
        {activeTab === 'analytics' && (
          <div className="grid grid-cols-1 gap-8">
             <div className="bg-white/5 border border-white/10 p-8 rounded-3xl overflow-hidden shadow-xl">
                <h3 className="text-xl font-bold mb-6 flex items-center gap-2">
                  <BarIcon className="text-emerald-400" /> Most Active Stations
                </h3>
                <div className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
